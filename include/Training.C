...
int BDT::Training(TCut cutSB, TString MC_DVCS, TString MC_Pi0, vector<TString> vars)
{
  ...
  TString outfileName(TString("TMVA.root"));
  ...
  TMVA::DataLoader *dataloader = new TMVA::DataLoader(TString("dataset"));
  ...
  dataloader->AddSpectator("_strip_Ph_Theta");
  dataloader->AddSpectator("_strip_Nuc_Theta");
  dataloader->AddSpectator("_miss_mom_eNg");
  ...
  //     dataloader->AddSignalTree( signalTree, signalTrainWeight, "Training" );
  //     dataloader->AddBackgroundTree( background, backgroundWeight, "Training" );
  //     dataloader->AddSignalTree( Data,     signalTestWeight,  "Test" );
  ...
  */
  ...
  // Check if the branch "Weight" exists in the tree
  if (background->GetBranch("Weight"))
  {
    dataloader->SetBackgroundWeightExpression("Weight");
  }

  // Apply additional cuts on the signal and background samples (can be different)
  //&& !TMath::IsNaN(delta_theta_p)
  ...
  dataloader->PrepareTrainingAndTestTree(cutSB, cutSB, "nTest_Signal=0:nTest_Background=0:SplitMode=Random:NormMode=NumEvents:!V");
  // dataloader->PrepareTrainingAndTestTree( cutSB, cutSB, "nTrain_Signal=1000000:nTrain_Background=1000000:nTest_Signal=1000000:nTest_Background=1000000:SplitMode=Random:NormMode=NumEvents:!V" );
  ...
  if (Use["BDTG"]) // Gradient Boost
    factory->BookMethod(dataloader, TMVA::Types::kBDT, "BDTG",
                        "!H:!V:NTrees=1000:MinNodeSize=2.5%:BoostType=Grad:Shrinkage=0.10:UseBaggedBoost:BaggedSampleFraction=0.5:nCuts=20:MaxDepth=2:!DoBoostMonitor");

  if (Use["BDT"]) // Adaptive Boost
    factory->BookMethod(dataloader, TMVA::Types::kBDT, "BDT",
                        "!H:!V:NTrees=850:MinNodeSize=2.5%:MaxDepth=3:BoostType=AdaBoost:AdaBoostBeta=0.5:UseBaggedBoost:BaggedSampleFraction=0.5:SeparationType=GiniIndex:nCuts=20:DoBoostMonitor");
  ...
  TString opt = "!V:!Silent:!Correlations"
    ...
    ":NumFolds=2"
    ":SplitType=Random";
  TMVA::CrossValidation cv{"K-fold", dataloader, outputFile , opt};
  ...
  */
  ...
  // gSystem->Exec(TString("mkdir -p ") + Folder + TString("dataset/"));
  // gSystem->Exec(TString("mv dataset ") + Folder);
  // gSystem->Exec(TString("mv TMVA.root ") + Folder + TString("TMVA.root"));
